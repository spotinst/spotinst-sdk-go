...
func main() {
	...
	out, err := svc.ReadStrategy(ctx, &oceancd.ReadStrategyInput{
		StrategyName: spotinst.String("strategy-for-datadog-cluster"),
	})
	if err != nil {
		log.Fatalf("spotinst: failed to read Strategy: %v", err)
	}
